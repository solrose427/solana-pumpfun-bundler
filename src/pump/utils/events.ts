...
export const toCreateEvent = (event: CreateEvent): CreateEvent => {
  return {
    name: event.name,
    symbol: event.symbol,
    uri: event.uri,
...
export const toCompleteEvent = (event: CompleteEvent): CompleteEvent => {
  return {
    user: new PublicKey(event.user),
    mint: new PublicKey(event.mint),
    bondingCurve: new PublicKey(event.bondingCurve),
...
export const toTradeEvent = (event: TradeEvent): TradeEvent => {
  return {
    mint: new PublicKey(event.mint),
...
export const toSetParamsEvent = (event: SetParamsEvent): SetParamsEvent => {
  return {
